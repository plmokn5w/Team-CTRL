...
class EXP(Object):
    def __init__(self, monster, path):
        ...
        self.speed = 15
        isHeart = random.randint(1, 50)
        self.heart = False
        if isHeart == 2:
            self.heart = True
            self.img = pygame.image.load(os.path.join(image_path, "heart" + ".png"))
            self.heartheal = 15
...
class EXPBar():
    ...
    def resize(self):
        self.img = pygame.transform.scale(self.img,
                                          (int(screen_width * (self.player.exp / self.player.exp_allot)) + 1, 30))
        self.rect = self.img.get_rect()
        self.rect.left = 0
        self.rect.top = 0
...
class Weapon:
    ...
    def shotBullet(self):
        if not self.atk():
            return
        p_x = self.player.x + self.bullet_cnt * 5
        p_y = self.player.y + self.bullet_cnt * 5
        target = None
        dist_min = 1000000
        ...
        for i in range(self.bullet_cnt):
            if i % 2 == 0:
                bullet_x = self.player.x + self.player.width*(i+1)/self.bullet_cnt
            bullet_y = self.player.y + self.player.height * (i + 1) / self.bullet_cnt
            Bullet((bullet_x, bullet_y), self.bullet, velocity, self.bullet_dmg, self.bullet_speed)
...
class Bullet(Object):
    ...
    def move(self):
        if math.sqrt((self.x - screen_width / 2 ) ** 2 + (self.y - screen_height / 2 ) ** 2) >= 800:
...
class Option:
    ...
    def enforce(self, player):
        ...
        if self.name == "enforce_option5":
            if player.weapons[0].attack_delay >= 4:
